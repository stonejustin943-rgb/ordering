...
async function submitOrder(){
  ...
  try{
    const resp = await fetch(DATA.meta.submitEndpoint, {
      ...
      // That triggers a CORS preflight which Apps Script web apps don't handle.
      body: JSON.stringify(payload)
    });
    const text = await resp.text();
    let out = {};
    try { out = JSON.parse(text); } catch (e) { out = { ok: false, error: text || "Non-JSON response" }; }
    msg.textContent=out.ok ? `Submitted! Saved to Drive. Ref: ${out.sheetName||"created"}` : `Error: ${out.error||"unknown"}`;
  }catch(err){
